...
import sijangkar.perhubungan.jatengprov.myapplication.fragmentation.exception.AfterSaveStateTransactionWarning;
import sijangkar.perhubungan.jatengprov.myapplication.fragmentation.helper.internal.ResultRecord;
import sijangkar.perhubungan.jatengprov.myapplication.fragmentation.helper.internal.TransactionRecord;
import sijangkar.perhubungan.jatengprov.myapplication.fragmentation.queue.Action;
import sijangkar.perhubungan.jatengprov.myapplication.fragmentation.queue.ActionQueue;
...
class TransactionDelegate {
    ...
    void destroyFragment(final FragmentManager fm, final ISupportFragment hideFragment) {
        enqueue(fm, new Action() {
            @Override
            public void run() {
                doDestroyFragment(fm, hideFragment);
            }
        });
    }

    void startCamera(final FragmentManager fm, final ISupportFragment showFragment, final ISupportFragment hideFragment) {
        enqueue(fm, new Action() {
            @Override
            public void run() {
                doStartCamera(fm, showFragment, hideFragment);
            }
        });
    }

    void stopCamera(final FragmentManager fm, final ISupportFragment showFragment, final ISupportFragment hideFragment) {
        enqueue(fm, new Action() {
            @Override
            public void run() {
                doStopCamera(fm, showFragment, hideFragment);
            }
        });
    }
...
//            ft.remove((Fragment) hideFragment);
            ft.replace(R.id.fl_container,(Fragment) showFragment);
        }
        supportCommit(fm, ft);
    }

    private void doDestroyFragment(FragmentManager fm, ISupportFragment hideFragment) {

        fm.beginTransaction().remove((Fragment) hideFragment).commitAllowingStateLoss();

    }

    private void doStartCamera(FragmentManager fm, ISupportFragment showFragment, ISupportFragment hideFragment) {



    }

    private void doStopCamera(FragmentManager fm, ISupportFragment showFragment, ISupportFragment hideFragment) {
